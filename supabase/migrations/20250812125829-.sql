...
create policy "Admins can view social accounts"
...
create policy "Users can view their brand social accounts"
  on public.social_accounts for select
  using (
    exists (
...
create policy "Users can view their brand posts"
  on public.posts for select
  using (
    exists (
      select 1 from public.users u
...
-- Admin policies
create policy "Admins can view post metrics"
  on public.post_metrics_daily for select
  using (public.is_admin_user());

create policy "Admins can insert post metrics"
  on public.post_metrics_daily for insert
  with check (public.is_admin_user());

create policy "Admins can update post metrics"
  on public.post_metrics_daily for update
  using (public.is_admin_user());

create policy "Admins can delete post metrics"
  on public.post_metrics_daily for delete
  using (public.is_admin_user());

-- Users can view metrics for posts belonging to their brand
create policy "Users can view their brand post metrics"
  on public.post_metrics_daily for select
  using (
    exists (
...
create policy "Admins can view content sync logs"
  on public.metricool_content_sync_logs for select
  using (public.is_admin_user());

create policy "Admins can insert content sync logs"
  on public.metricool_content_sync_logs for insert
  with check (public.is_admin_user());

-- 5) Update triggers for updated_at columns (reuse existing function)
